    ...
    <div className="max-w-2xl space-y-8">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('onboarding.step0.title')}</label>
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('settings.bookingLink')}</label>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="space-y-4">
          {([
            { key: 'sms_on_new',    label: t('settings.smsOnNew') },
            { key: 'sms_on_cancel', label: t('settings.smsOnCancel') },
            { key: 'sms_reminder',  label: t('settings.smsReminder') },
          ] as const).map(item => (
          ...
          <button onClick={() => saveNotif.mutate()} disabled={saveNotif.isPending}
            className="bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 text-white text-sm font-medium px-5 py-2.5 rounded-lg">
            {savedNotif ? t('common.saved') : saveNotif.isPending ? t('common.saving') : t('common.save')}
          </button>
        </div>
      </div>
      ...
      {/* QR Kod */}
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <h2 className="font-semibold text-gray-900 mb-1">{t('settings.qrCode')}</h2>
        <button onClick={downloadQr} disabled={qrLoading}
          ...
          {qrLoading ? t('common.downloading') : t('settings.downloadQr')}
        </button>
      </div>

      {/* Destek Talebi */}
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <h2 className="font-semibold text-gray-900 mb-1">{t('settings.support')}</h2>
        <div className="space-y-3 mt-3">
          <div>
            <input
              value={ticket.subject}
              onChange={e => setTicket(tk => ({ ...tk, subject: e.target.value }))}
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
          ...
          <div>
            <textarea
              ...
              onChange={e => setTicket(tk => ({ ...tk, message: e.target.value }))}
              rows={4}
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
              placeholder={t('settings.supportMessagePlaceholder')}
            />
          </div>
          {ticketSent && <p className="text-green-600 text-sm">✓ {t('common.sent')}</p>}
          <button
            onClick={() => sendTicket.mutate()}
            disabled={sendTicket.isPending || ticket.subject.length < 3 || ticket.message.length < 10}
            className="bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 text-white text-sm font-medium px-5 py-2.5 rounded-lg"
          >
            {sendTicket.isPending ? t('common.sending') : t('settings.sendTicket')}
          </button>
        </div>
      </div>
    </div>
