...
function formatBlockLabel(b: any) {
  ...
  if (sameDay && fullDay) return { date: fmt(start), sub: 'fullday',  type: 'fullday'  as BlockType };
  if (sameDay)            return { date: fmt(start), sub: `${time(start)} – ${time(end)}`, type: 'hours' as BlockType };
  return                         { date: `${fmt(start)} – ${fmt(end)}`, sub: 'multiday', type: 'multiday' as BlockType };
}

function TimeInput({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500 bg-white transition-shadow">
      <div className="px-3 py-2.5 bg-gray-50 border-r border-gray-200 shrink-0">
        <Clock className="w-4 h-4 text-gray-400" />
      </div>
      <input type="time" value={value} onChange={e => onChange(e.target.value)}
        className="flex-1 px-3 py-2.5 text-sm outline-none bg-white text-gray-800 min-w-0 [&::-webkit-calendar-picker-indicator]:hidden" />
    </div>
  );
}
...
export default function StaffWorkingHoursPage() {
  ...
  const saveHours = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['my-working-hours'] }); setSaved(true); setTimeout(() => setSaved(false), 2500); },
  });

  const { data: blocksData } = useQuery({
    queryKey: ['my-blocked-periods', staffId],
  ...
  const addBlock = useMutation({
    mutationFn: () => {
      ...
      const start_at = new Date(blockType === 'hours' ? `${date}T${startTime}:00` : `${date}T00:00:00`).toISOString();
      const end_at = new Date(
        blockType === 'hours'    ? `${date}T${endTime}:00` :
        blockType === 'multiday' ? `${endDate}T23:59:59`   : `${date}T23:59:59`
      ).toISOString();
      return api.post(`/staff/${staffId}/blocked-periods`, { start_at, end_at, reason: blockForm.reason || undefined });
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['my-blocked-periods'] }); setBlockForm(emptyBlockForm()); setBlockOpen(false); },
  });
  ...
  const BLOCK_TYPES = [
    { key: 'hours' as BlockType,    label: t('workingHours.blockTypes.specificHours') },
    { key: 'fullday' as BlockType,  label: t('workingHours.blockTypes.fullDay') },
    { key: 'multiday' as BlockType, label: t('workingHours.blockTypes.vacation') },
  ];

  if (isLoading) return <div className="flex justify-center py-16"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-500" /></div>;
    ...
    <div className="max-w-xl">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 mb-5 overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100">
          <p className="font-semibold text-gray-900">{t('workingHours.title')}</p>
        </div>
          ...
          {hours.map((h, i) => (
            <div key={i} className="flex items-center justify-between gap-2 px-4 py-3">
              <div className="flex items-center gap-2.5 min-w-0">
                <label className="relative inline-flex items-center cursor-pointer shrink-0">
                  <input type="checkbox" checked={h.is_open}
                    onChange={e => {
                      ...
                      updateHour(i, { is_open: open, ...(open && !h.start_time ? { start_time: '09:00', end_time: '18:00' } : {}) });
                    }}
                ...
                <span className="text-sm font-medium text-gray-700 truncate">{t(`common.days.${DAY_KEYS[h.day_of_week]}`)}</span>
              </div>
              {h.is_open ? (
                <div className="flex items-center gap-1.5 shrink-0">
                  <input type="time" value={h.start_time ?? '09:00'} onChange={e => updateHour(i, { start_time: e.target.value })}
                    className="w-[76px] border border-gray-200 rounded-lg px-2 py-1.5 text-sm outline-none bg-white text-gray-800 focus:ring-2 focus:ring-indigo-500 [&::-webkit-calendar-picker-indicator]:hidden" />
                  <span className="text-gray-300 text-xs">–</span>
                  <input type="time" value={h.end_time ?? '18:00'} onChange={e => updateHour(i, { end_time: e.target.value })}
                    className="w-[76px] border border-gray-200 rounded-lg px-2 py-1.5 text-sm outline-none bg-white text-gray-800 focus:ring-2 focus:ring-indigo-500 [&::-webkit-calendar-picker-indicator]:hidden" />
                </div>
              ) : (
        ...
        <div className="px-5 py-4 bg-gray-50 border-t border-gray-100">
          <button onClick={() => saveHours.mutate()} disabled={saveHours.isPending}
            ...
            {saved ? <><Check className="w-4 h-4" /> {t('common.saved')}</> : saveHours.isPending ? t('common.saving') : t('workingHours.save')}
          </button>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          ...
          {!blockOpen && (
            <button onClick={() => setBlockOpen(true)}
              ...
              <Plus className="w-4 h-4" /> {t('workingHours.addBlock')}
            </button>
          )}
        </div>
        ...
        {blockOpen && (
          <div className="px-5 py-4 border-b border-gray-100 bg-slate-50 space-y-4">
            <div className="grid grid-cols-3 gap-1.5">
              {BLOCK_TYPES.map(bt => (
                <button key={bt.key} onClick={() => setBlock({ blockType: bt.key })}
            ...
            {blockForm.blockType === 'multiday' ? (
              ...
              <div>
                <p className="text-xs font-medium text-gray-500 mb-1.5">{t('booking.selectDate')}</p>
                <DatePicker value={blockForm.date} min={today} onChange={v => setBlock({ date: v })} />
              </div>
            )}
            ...
            {blockForm.blockType === 'hours' && (
              <div>
                <p className="text-xs font-medium text-gray-500 mb-1.5">{t('booking.selectTime')}</p>
                <div className="flex items-center gap-2">
                  <TimeInput value={blockForm.startTime} onChange={v => setBlock({ startTime: v })} />
                  <span className="text-gray-300 shrink-0 text-lg">–</span>
                  <TimeInput value={blockForm.endTime} onChange={v => setBlock({ endTime: v })} />
                </div>
              </div>
            )}

            <div>
              <p className="text-xs font-medium text-gray-500 mb-1.5">{t('workingHours.reasonLabel')} <span className="font-normal text-gray-400">({t('common.optional')})</span></p>
              <input value={blockForm.reason} onChange={e => setBlock({ reason: e.target.value })}
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={() => addBlock.mutate()} disabled={addBlock.isPending || !canSave}
              ...
              <button onClick={() => { setBlockOpen(false); setBlockForm(emptyBlockForm()); }}
            ...
            {addBlock.isError && <p className="text-red-600 text-xs">{(addBlock.error as any)?.response?.data?.message ?? t('common.errorOccurred')}</p>}
          </div>
        )}
        ...
        {blocks.length === 0 ? (
          ...
          <ul className="divide-y divide-gray-100">
            {blocks.map((b: any) => {
              ...
              const subLabel = sub === 'fullday' ? t('staff.blockLabel.fullDay') : sub === 'multiday' ? t('staff.blockLabel.vacation') : sub;
              return (
                <li key={b.id} className="flex items-center justify-between px-5 py-3.5">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-400">{subLabel}{b.reason ? ` · ${b.reason}` : ''}</p>
                    </div>
                  </div>
                  <button onClick={() => removeBlock.mutate(b.id)} disabled={removeBlock.isPending}
