  ...
  return (
    ...
    <div
      ...
      className={`bg-white rounded-2xl overflow-hidden border ${
        isPendingStatus ? 'border-amber-300' : 'border-gray-200'
      }`}
    >
      ...
      <div className="flex items-center gap-3 px-4 pt-3 pb-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            <p className="font-semibold text-gray-900 text-sm leading-none truncate">{a.customer_name}</p>
            <span className={`shrink-0 text-[10px] px-1.5 py-0.5 rounded-full font-medium ${STATUS_COLOR[a.status] ?? ''}`}>
              {statusLabel}
            </span>
          </div>
        ...
        <button onClick={() => setOpen(o => !o)}
          className="shrink-0 p-1.5 rounded-xl border border-gray-200 hover:bg-gray-50 text-gray-400 transition-colors">
      ...
      <div
        style={{
          maxHeight: open ? (detailRef.current?.scrollHeight ?? 600) + 'px' : '0px',
          overflow: 'hidden',
          transition: 'max-height 0.28s ease',
        }}
      >
        <div ref={detailRef} className="px-4 pb-4 pt-1 border-t border-gray-100">
